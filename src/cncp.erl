...
-export([map/2, cmap/2, cmap_limit/3, 
         ...
         parallel_collect/1, parallel_collect/2, cparallel_collect/1, cparallel_collect/2, cparallel_collect_limit/2, cparallel_collect_limit/3]).

-export([parallel_t/0, t/0]).
...
parallel_collect(FunList) ->
    cncp_parallel_collect:parallel_collect(FunList).

parallel_collect(FunList, Args) ->
    cncp_parallel_collect:parallel_collect(FunList, Args).

cparallel_collect(FunList) ->
    cncp_parallel_collect:cparallel_collect(FunList).

cparallel_collect(FunList, Args) ->
    cncp_parallel_collect:cparallel_collect(FunList, Args).

cparallel_collect_limit(FunList, Limit) ->
    cncp_parallel_collect:cparallel_collect_limit(FunList, Limit).

cparallel_collect_limit(FunList, Args, Limit) ->
    cncp_parallel_collect:cparallel_collect_limit(FunList, Args, Limit).
...
parallel_t() ->
    ...
    cncp:parallel_collect([One, Two, Three, Four, Five, Six], [5]),
    
    io:format("~n~n parallel ~n~n"),
    cncp:cparallel_collect([One, Two, Three, Four, Five, Six], [5]),

    io:format("~n~n limit ~n~n"),
    cncp:cparallel_collect_limit([One, Two, Three, Four, Five, Six], [5], 3).
...
t() ->
    ...
    io:format("~n~n limit ~n~n"),
    cncp:cparallel_limit([One, Two, Three, Four, Five, Six], [5], 2).